...
def decode_chess(estimator, hparams, decode_hp, checkpoint_path=None):
  ...
  def input_fn(): #a function, which, when run, generates input...
    gen_fn = make_input_fn_from_generator(
        _interactive_input_fn(hparams, decode_hp, Q))
    example = gen_fn()
  ...
  TESTMODE = False
  TWOPLAYER = False
  HUMANIS = 1 #0 WHITE, 1 BLACK


  test_games = []
  with open('xxx/dump/shuffled_test','r') as f:
    while len(test_games) < 50:
      line = f.readline()
      if line[0] != 'B':
        continue
  ...
  while 1: 
    ...
    if TESTMODE:
      ...
      test_game = test_games.pop()
      print('***'*10)
      print (test_game)
      #test_game.pop()
      test_game = test_game[::-1]
      moves_so_far = [test_game.pop()]
      total_logprob = 0.0
      ...
      #here's how to start with a partial game
      #the bongcloud...
      # moves_so_far = ['A','e2e4','e7e5','e1e2','b8c6']

      # if len(moves_so_far) > 1:
      #   for m__ in moves_so_far[1:]:
      #     m__ = chess.Move.from_uci(m__)
      #     board.push(m__)
      #     node = node.add_main_variation(m__)
          
    while 1:

      if not TESTMODE and ((not TWOPLAYER) or (len(moves_so_far) % 2) == HUMANIS):
        Q.put(' '.join(moves_so_far))
        result = list(estimator.predict(input_fn, checkpoint_path=checkpoint_path))[0]
        logits = result['logits'][0,0,0] #1973
        #un-normed

        targets_vocab = hparams.problem_hparams.vocabulary["targets"]
        #we need to obtain a VALID MASK here...

        valid_moves = [str(x) for x in board.legal_moves]

        if board.is_repetition(3):
          #disallow the move which would cause rep.
          print('DISALLOWING REP')
          disallowed_move = moves_so_far[-4] #what 'i' played 2 moves ago

          assert (disallowed_move in valid_moves)
          valid_moves.remove(disallowed_move)

        valid_indices = targets_vocab.encode(' '.join(valid_moves))


        if len(moves_so_far) > 40 or (not valid_moves):
          valid_indices.extend([0,1,2])

        mask = np.zeros_like(logits)

        mask[np.array(valid_indices)] = 1.0

        assert (mask.sum() >= 0.999)

        if len(moves_so_far) < 10:
          SAMPLING_TEMP = 1.0
        elif len(moves_so_far) < 20:
          SAMPLING_TEMP = 0.25
        elif len(moves_so_far) < 40:
          SAMPLING_TEMP = 0.1
        ...
        logits = logits / SAMPLING_TEMP

        logits[np.array(valid_indices)] += 1000.0
        #convert to probabilities...
        logits = logits - logits.max() 
        denom = np.sum(np.exp(logits) * mask)
        numer = np.exp(logits)*mask
        probs = numer/denom - 1E-8
        # assert (probs.sum() < 1+1E-3) #sometimes we can go over a tiny bit... need to fix
        # probs = probs / probs.sum()
        print (probs.sum()-1.0)

        sample = np.random.multinomial(1, probs)

        move_str = targets_vocab.decode([sample.argmax()])
      elif not TESTMODE:
        flag = False
        while not flag:
          move_san = input('Your Turn: ') #SAN
          try:
            mymove = board.parse_san(move_san) #check legality
            move_str = mymove.uci()
            print(move_str)
            flag = True
          except:
            print('move seems invalid, try again')
      else:
        # if test_game:
        #   move_str = test_game.pop()
        # else:
        #   print('FINISHED')
        #   print('moves:', len(moves_so_far)-1)
        #   print('log p:', total_logprob)
        #   print('avglp:', total_logprob / (len(moves_so_far)-1))
          #return #DONE
        move_str = test_game.pop()
        ...
        log_prob = logits - np.log(np.sum(np.exp(logits) * mask))
        #log_prob = logits - np.log(np.sum(np.exp(logits)))
        print (np.exp(log_prob).sum()-1.0)

        log_prob_idx = targets_vocab.encode(move_str)[0]
        log_prob_move = log_prob[log_prob_idx]
        total_logprob += log_prob_move
      ...
      if move_str in '012':
        if TESTMODE:
          total_logprob -= log_prob_move #don't count this!!
        ...
        if TESTMODE:
          total_moves += len(moves_so_far)-2
          total_logp += total_logprob
  ...
  avg_nats = -total_logp/total_moves
  avg_bits = avg_nats / np.log(2)
  perp = np.exp(avg_nats)
  print (total_moves, total_logp, avg_nats, avg_bits, perp)
